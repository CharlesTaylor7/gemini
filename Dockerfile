...
RUN stack install --resolver lts.18-14 aeson servant-server wai text warp mtl transformers parsec
...
RUN stack setup && stack install --resolver lts.18-14 --dependencies-only 
...
COPY --from=0 /reversible-debugger/.stack-work/install/x86_64-linux/lts.18-14/8.0.2/bin/server /app/server
